...
class FitsDict():
    ...
    def __init__(self, options, train_dataset):
        ...
        for ds_name, ds in train_dataset.dataset_dict.items():
            try:
                # try load final_fit of SPIN
                if ds_name == 'h36m':
                    # for Human3.6M, final_fit labels are ground-truths
                ...
                dict_file = os.path.join(path_config.STATIC_FITS_DIR, ds_name + '_fits.npy')
                self.fits_dict[ds_name] = torch.from_numpy(np.load(dict_file))
