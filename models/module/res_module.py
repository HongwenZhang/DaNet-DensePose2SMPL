...
class IUV_predict_layer(nn.Module):
    def __init__(self, feat_dim=256, final_cov_k=3, part_out_dim=25):
        ...
        self.predict_hm = nn.Sequential(self._make_layer(Bottleneck, int(feat_dim / 4), 3, stride=1),
                                        nn.Conv2d(feat_dim, 24, kernel_size=3, stride=1, padding=1, bias=True))

        if cfg.DANET.INPUT_MODE == 'iuv_feat':
            self.global_feat_down = nn.Sequential(
                nn.Conv2d(
                    in_channels=feat_dim,
                    out_channels=64,
                    kernel_size=final_cov_k,
        ...
        if cfg.DANET.DECOMPOSED:
            self.predict_partial_iuv = nn.Conv2d(
                in_channels=feat_dim * 24,
                out_channels=part_out_dim * 3 * 24,  # 5 -> 25 -> part_out_dim
                kernel_size=final_cov_k,
                stride=1,
                padding=1 if final_cov_k == 3 else 0,
                groups=24
            ...
            if cfg.DANET.INPUT_MODE == 'iuv_feat':
                self.part_feat_down = nn.Sequential(
                    nn.Conv2d(
                        in_channels=feat_dim * 24,
                        out_channels=64 * 24,
                        kernel_size=final_cov_k,
    ...
    def forward(self, x):
        ...
        predict_hm = self.predict_hm(x)
        return_dict['predict_hm'] = predict_hm
