'''
    code brought in part from:
    file:   https://github.com/MandyMo/pytorch_HMR/blob/master/src/util.py
    date:   2018_04_29
...
def reflect_lsp_kp(kps):
    if len(kps) == 14:
        ...
        kp_map = [0, 4, 5, 6, 1, 2, 3, 7, 8, 9, 10, 14, 15, 16, 11, 12, 13]
    elif len(kps) == 24:
...
def axis_angle_add(theta, roll_axis, alpha):
    ...
    if False:
        a_cos, b_cos = b_cos, a_cos
        a_sin, b_sin = b_sin, a_sin
        roll_axis, normalized = normalized, roll_axis

    dot_mm = torch.sum(normalized * roll_axis, dim=1).unsqueeze(1)
    cross_mm = torch.zeros(normalized.size())
...
def flip_image(src_image, kps, box=None):
